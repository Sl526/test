...
class video_listpage(SiteHeader, ExecutorBase):
    ...
    def click_video_menu(self):
        try:
            ...
            logger.warning(f"菜单点击失败，将继续执行：{str(e)}")
    ...
    def click_main_stream0_encode_type(self):
        try:
            ...
            logger.warning(f"编码类型点击失败，将继续执行：{str(e)}")
    ...
    def select_stream0_encode_type(self, encode_type):
        ...
        if encode_type not in encode_map:
            logger.error(f"不支持的编码类型：{encode_type}，仅支持H264/H265")
            return 
        try:
            self._click_element((By.XPATH, encode_map[encode_type]))
            
            logger.info(f"成功选择编码类型：{encode_type}")
        except (NoSuchElementException, TimeoutException) as e:
            logger.error(f"选择编码类型 {encode_type} 失败：{str(e)}")
    ...
    def click_main_stream0_resolution(self):
        try:
            ...
            logger.warning(f"分辨率下拉框点击失败：{str(e)}")
    ...
    def select_stream1_encode_type(self, encode_type):
        ...
        if encode_type not in encode_map:
            logger.error(f"不支持的编码类型：{encode_type}，仅支持H264/MJPEG/H265")
            return 
        try:
            self._click_element((By.XPATH, encode_map[encode_type]))
            logger.info(f"成功选择编码类型：{encode_type}")
    ...
    def select_resolution(self, resolution):
        if resolution in self.RESOLUTION_MAP:
            try:
                ...
                logger.info(f"成功选择分辨率：{resolution} (MAIN_STREAM)")
                return True
            except (NoSuchElementException, TimeoutException) as e:
                logger.error(f"选择分辨率 {resolution} (MAIN_STREAM) 失败：{str(e)}")
        
        if resolution in self.RESOLUTION_SUP:
            try:
                self._click_element((By.XPATH, self.RESOLUTION_SUP[resolution]))
                logger.info(f"成功选择分辨率：{resolution} (SUB_STREAM)")
                return True
            except (NoSuchElementException, TimeoutException) as e:
                logger.error(f"选择分辨率 {resolution} (SUB_STREAM) 失败：{str(e)}")
        
        logger.error(f"不支持的分辨率：{resolution}，支持的分辨率：{list(self.RESOLUTION_MAP.keys()) + list(self.RESOLUTION_SUP.keys())}")
        return False
    ...
    def click_ok_button(self):
        try:
            ...
            logger.warning(f"确认按钮点击失败：{str(e)}")
        ...
        for idx, resolution in enumerate(self.RESOLUTION_MAP.keys()):
            try:
                ...
                if self.select_resolution(resolution):
                    ...
                    logger.info(f"完成{encode_type}编码-{resolution}分辨率的设置")
            except Exception as e:
                logger.error(f"设置{encode_type}编码-{resolution}分辨率失败：{str(e)}")
                continue 
    ...
    def set_all_video_configs(self):
        try:
            ...
            logger.critical(f"STREAM0_H264 批量设置过程出现未预期错误：{str(e)}")

        try:
            self.set_video_stream0_resolution_for_encode_type("STREAM0_H265")  # 修正参数
        except Exception as e:
            logger.critical(f"STREAM0_H265 批量设置过程出现未预期错误：{str(e)}")

        try:
            self.set_video_stream1_resolution_for_encode_type("STREAM1_H264")  # 修正参数
        except Exception as e:
            logger.critical(f"STREAM1_H264 批量设置过程出现未预期错误：{str(e)}")

        try:
            self.set_video_stream1_resolution_for_encode_type("STREAM1_MJPEG")  # 修正参数
        except Exception as e:
            logger.critical(f"STREAM1_MJPEG 批量设置过程出现未预期错误：{str(e)}")

        try:
            self.set_video_stream1_resolution_for_encode_type("STREAM1_H265")  # 修正参数
        except Exception as e:
            logger.critical(f"STREAM1_H265 批量设置过程出现未预期错误：{str(e)}")
            
        logger.info("所有视频配置尝试已完成（无论成功或失败）")